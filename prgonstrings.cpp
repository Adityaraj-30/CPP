...
int main() {
    ...
    cin.getline(arr2,80); //.getline is used to include the whitespaces in the entered string as a part of string. else cin stops at the first whitespace encountered after a character
...
void switch_2(char arr1[],char arr2[]) {
    string s1=arr1,s2=arr2; //character array  is first initialized to string s1 and s2 respectively to use string member functions like .append,strncat
    string s3=s1+s2;
    cout << s3;
    return;
}
...
void switch_3(char arr1[],char arr2[]) {
    char* temp1=arr1;
    temp1=arr1;
    strcat(temp1,arr2); //arr1 and arr2 are c strings thus we can use strcat directly
    cout <<temp1;
    return;
}
...
void switch_9 (char arr1[],char arr2[]) {
    ...
    char rarr1[len1+1]={'\0'},rarr2[len2+1]={'\0'}; //if we dont declare array of 1 size greater than the actual array then at the time of printing it will print a junk last charecter.
    int j=len1-1;
